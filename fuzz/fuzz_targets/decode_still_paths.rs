...
//! the container *declares* before any decode runs (the shared
//! [`oxideav_webp_fuzz::declared_pixel_load`] helper: top-level §2.6
...
//! §2.6 / §2.5 sub-bitstream dimensions, each of which decodes at the
...
//! decoding at all — there is no cheap "still drive the façade for the
//! Result contract" arm here, because the over-budget allocation *is*
//! the success path of the two façades under test.

use libfuzzer_sys::fuzz_target;
use oxideav_webp::{container, decode_webp, decode_webp_image, DecodedWebp};
use oxideav_webp_fuzz::{declared_pixel_load, MAX_DECLARED_PIXELS};
